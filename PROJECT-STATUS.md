...
> **Last updated:** 2026-03-24 (session 53 — fixed the shared navbar dropdown regression by removing masthead clipping in `src/app/layout.tsx`, removing the nav scroll wrapper in `src/components/layout/NavigationBar.tsx`, and letting the full submenu panels render above page content)
...
- **Local Life** (`/local-life`): dynamic DB-backed categories. Current parent categories are `News`, `Community`, `Lifestyle`, `Opinion`, and `Announcements`. `/local-life` filter pills come from `/api/categories?top=true`; the submit page fetches all categories and groups subcategories with `<optgroup>`.
...
Subcategory links use `?category=slug` query params matching `Category.slug`. Nav data is still hardcoded in `NavigationBar.tsx` and should later move to a DB-backed source. `NavigationBar` uses `useSession` to gate the `Arcade` link. Legacy `/articles` still exists for backward compatibility.
...
src/app/
  ...
  roadmap/               # list, detail, submit, manage, ballot ranking
...
- The public `About` area is live with `Mission`, `Roadmap`, and `Journal`. Mission copy now uses a warm first-person-plural voice rooted in Cambria Heights and the highlands of Cambria County.
...
- The static `About` section is complete. Remaining follow-up is richer Journal/history structure and future pages such as `Transparency` and `Support`.
...
- **Shared internal page heroes:** `src/components/shared/InternalPageHeader.tsx` is now the standard compact red-gradient page hero/section-header component for the slimmer internal/public top-of-page treatment. It supports an optional action area plus per-page title color override, and the uppercase title label was increased from `text-xs` to `text-sm` for better visibility. `Local Life`, `Messages`, `My Articles`, `Roadmap` manage/submit, `Help Wanted` manage/submit, `My Stores`, `Create a Store`, `Events`, `Marketplace`, `Profile` edit, and `Experiences` now use this pattern.
...
- **Homepage:** `src/app/page.tsx` + `src/lib/homepage.ts` SSR-render the homepage using `HomepageSection` / `HomepagePinnedItem`. It resolves community by domain/host with localhost fallback, excludes featured stories from Latest News, uses `next/image`, and removed the old widget components. The live homepage uses the `Youth Local` visual direction.
...
- **Directory planning:** `DIRECTORY-PLAN.md` exists. Current direction: people listings are opt-in, expose no phone/address, route contact through internal messaging only, restrict message initiation to `TRUSTED` users, omit organization memberships from user directory entries, and let organizations choose whether to publish selected members.
...
- **Community roadmap:** `RoadmapIdea`, `RoadmapRankingBallot`, and `RoadmapRankingItem` are in the schema, with roadmap-specific permissions and activity-log resource types. `/api/roadmap`, `/api/roadmap/[id]`, `/api/roadmap/[id]/moderate`, and `/api/roadmap/ballot` support trusted submission/edit/delete/resubmit, moderation/merge, ordered top-five ballot saving, leaderboard aggregation, and public browsing of approved/planned/in-progress/shipped ideas. Public routes are `/roadmap`, `/roadmap/[id]`, `/roadmap/submit`, and `/roadmap/manage`; staff moderation is at `/admin/roadmap`.
- **Roadmap weighting foundation:** `DomainInfluenceWeight` and `InfluenceDomain` are in the schema. The first live domain is `ROADMAP_FEATURE_PRIORITIZATION`. `/api/admin/roadmap/weights` lets admins set bounded roadmap-only multipliers with required rationale; changes are logged as `DOMAIN_INFLUENCE_WEIGHT`. The public roadmap leaderboard applies these weights.
- **Roadmap weighting transparency:** `/roadmap` shows weighting bounds plus weighted vs raw points. The admin roadmap page shows recent roadmap-weight changes from `ActivityLog`, and `/api/admin/activity-logs` now recognizes `ROADMAP_IDEA`, `ROADMAP_RANKING_BALLOT`, and `DOMAIN_INFLUENCE_WEIGHT`.
...
- **Admin layout:** `src/app/admin/layout.tsx` intentionally breaks out of the shared public container and uses full viewport width. The sidebar was softened from near-black to a lighter slate treatment.
- **About baseline:** `/about`, `/about/mission`, `/about/roadmap`, and `/about/journal` are live. Mission copy is now warmer and more specific; pillar cards, nav descriptions, and journal entry summaries match that tone. Content still lives in `src/lib/about.ts` and page JSX; there is no CMS/data model yet. About pages use the active `Youth Local` language.
- **Security logging:** login events and immutable activity logs are live, including admin endpoints for investigation/export.
- **Repository / deployment baseline:** git is live, `main` is pushed to `https://github.com/dsjrego/highlander-today`, `README.md` reflects the current product surface, `.env.example` uses Docker Postgres on `127.0.0.1:5433`, and `src/app/api/health/route.ts` now exists for Docker `HEALTHCHECK`.
- **Production environment:** production domain is `https://highlander.today`; registrar remains Namecheap; authoritative DNS is now Cloudflare; hosting is Vercel. Cloudflare R2 uploads are live on `https://cdn.highlander.today`. MaxMind login geolocation is live in production. Active launch auth providers are credentials plus Google OAuth. Facebook OAuth is intentionally deferred until Meta business verification is complete. `npm run deploy:check-env` now validates deploy-critical env vars. Locked-profile photo/bio updates were fixed by submitting only editable fields and comparing actual changes rather than field presence.
...
36. **Community roadmap direction:** roadmap input is curated: trusted users submit, staff normalize/merge/moderate, and the community ranks approved ideas. This informs, but does not replace, final editorial/product judgment.
...
40. **Platform sequencing:** intended order remains `(1) trust/content/community identity`, `(2) store-based listings/discovery`, `(3) Help Wanted`, `(4) community feature prioritization`, `(5) domain-specific weighting/reputation`, `(6) delivery/jobs`, `(7) transaction infrastructure such as payments, inventory, shipping, merchant APIs`. Milestones 1-4 are complete for the current MVP loops.
41. **Current execution priority:** deployment is complete enough that engineering focus should now be post-launch validation of live loops plus the `About` track, not speculative delivery/jobs work or more Help Wanted polish without user feedback.
...
3. **Milestone 3 — Community feature-prioritization MVP:** complete. Trusted users can submit ideas, staff can moderate/merge them, approved ideas enter a ranking pool, and trusted users can submit ordered priorities that roll into a community-informed leaderboard. Remaining work is polish, duplicate-merge ergonomics, and richer history/context if needed.
4. **Milestone 4 — Domain-specific weighting/reputation MVP:** complete. The roadmap-first weighting foundation is live with bounded per-user multipliers, required rationale, auditable activity logs, active-community history, shared policy constants for the current `90%-110%` range, and public weighted-vs-raw transparency.
5. **Milestone 5 — Article video embeds MVP:** safe YouTube/Vimeo embeds inside TipTap without expanding into native video upload/storage.
6. **Milestone 6 — Delivery jobs MVP:** requester posts a delivery job, eligible drivers can browse/receive it based on assignment mode, one driver can claim/accept it, and the job moves through basic completion states.
7. **Milestone 7 — Decide next bottleneck:** only after milestones 1-6 are stable should the next expansion lane be chosen among payments, restaurant ordering, merchant tooling, or broader integrations.
...
- **Roadmap ranking:** verified ordered-ballot behavior through repo-wide type checking and the public roadmap/ballot surface. Trusted users can save one ordered top-five ballot and the public leaderboard is visible.
- **Roadmap weighting:** verified Prisma client regeneration, repo-wide type checking, admin/public roadmap code paths, weighted-vs-raw transparency, and the native recent-change view in admin. Local DB schema application still depends on the same manual-Postgres fallback if Prisma’s schema engine fails.
