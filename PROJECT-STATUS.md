...
> **Last updated:** 2026-03-25 (session 57 — expanded the shared `.empty-state-title` treatment in `src/app/globals.css` so it now controls font family, responsive size, weight, and spacing in addition to the branded gradient color; removed conflicting per-element heading utilities from current uses; and applied the same shared title treatment to the About Mission hero line so it matches the homepage and other public empty-state headings)
...
5. **Milestone 5 — Organization foundation + directory/org-site MVP:** add a first-class `Organization` model and `OrganizationMembership` join model, explicitly separate from `Store`. This foundation should power the organizations side of the planned white-pages/yellow-pages-style directory, support accountable trusted ownership/management, organization-level public roster controls, and leave room for paid host/subdomain routing such as `flavortown.highlander.today` without treating marketplace stores as the general organization abstraction.
6. **Milestone 6 — Article video embeds MVP:** safe YouTube/Vimeo embeds inside TipTap without expanding into native video upload/storage.
7. **Milestone 7 — Delivery jobs MVP:** requester posts a delivery job, eligible drivers can browse/receive it based on assignment mode, one driver can claim/accept it, and the job moves through basic completion states.
8. **Milestone 8 — Decide next bottleneck:** only after milestones 1-7 are stable should the next expansion lane be chosen among payments, restaurant ordering, merchant tooling, or broader integrations.
