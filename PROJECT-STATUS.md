...
> **Last updated:** 2026-03-25 (session 61 — relabeled the About dropdown entry from `Journal` to `Blog`, added a Super Admin-only `Support` navbar dropdown with placeholder support routes, and updated documentation to reflect the current nav state)
...
- **About**: top-level nav dropdown separate from Local Life and Experiences. Live subpages are `Mission`, `Roadmap`, and `Blog` (the current route remains `/about/journal`). Mission copy uses a warm first-person-plural voice grounded in Cambria Heights and the highlands of Cambria County, positioning the platform as community infrastructure and expansion as “get it right here first.”
