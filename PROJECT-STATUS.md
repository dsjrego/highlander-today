...
> **Last updated:** 2026-04-08 (session 129)
...
- Admin recurring events now have a first scoped management loop on `/admin/events/[id]`: staff can edit or delete just the current occurrence, the current and future occurrences, or the entire series for non-cadence fields, and the API keeps session numbering/counts in sync afterward.
...
- Event-series editing remains partial; admin flows now support `this occurrence` versus `future` versus `whole series` for non-cadence edits/deletes, but full cadence regeneration and series-wide schedule editing semantics are still pending.
