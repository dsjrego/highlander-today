...
> **Last updated:** 2026-03-24 (session 47 — fixed global spacing below the public navbar, removed page-level width caps that were fighting the shared container, corrected the custom `.container` CSS override, and expanded/lightened the admin layout so admin screens use full viewport width with a softer sidebar)
...
Multi-tenant community platform currently anchored in **Cambria Heights** — the boroughs and townships of the Cambria Heights School District area in **Cambria County, Pennsylvania**. The platform provides local content, events, a store-based market/ecommerce platform, help wanted (services/jobs postings), a reusable delivery/job marketplace, and private messaging with a trust-based membership model (members vouch for new members; revocation cascades via BFS). The long-term vision is community infrastructure that functions more like a communication utility than a commercial enterprise — each community gets its own locally-rooted space via multi-tenancy, but communities connect to one another, so local never means isolated. Cambria Heights is the proving ground: get it right here first, then expand.
...
**About** (live top-level nav dropdown): institutional/platform section, separate from Local Life and Experiences. Live subpages are `Mission`, `Roadmap`, and `Journal`. The mission page uses a warm, first-person-plural voice grounded in Cambria Heights and the highlands of Cambria County, framing the platform as community infrastructure (utility, not commercial enterprise) with a "get it right here first" expansion philosophy. Future public pages such as `Transparency` and `Support` may eventually live under this umbrella as well.

Nav: Home, Local Life (dropdown), Experiences (dropdown), Market, Help Wanted, About (live), Arcade (Super Admin only). Subcategory links use `?category=slug` query params matching `Category.slug` in DB. Nav data hardcoded in `NavigationBar.tsx` — swap to DB fetch when wiring real data. NavigationBar uses `useSession` to check role and conditionally render the Arcade link. Old `/articles` route exists for backwards compatibility.
...
- **Public institutional voice direction:** The `About` area is now live with `Mission`, `Roadmap`, and `Journal` pages. The mission page copy has been rewritten to use a warm, first-person-plural ("we") voice grounded in Cambria Heights rather than the original third-person institutional tone. The public voice positions Highlander Today as community infrastructure — technology that works more like a utility than a commercial product — and names the highlands of Cambria County as the proving ground before multi-community expansion.
- **Technology-as-infrastructure philosophy:** The core mission framing is that local communication, coordination, discovery, and market-access technology functions like essential infrastructure. The product should aim for broad usefulness, accountable operation, transparent economics, and non-extractive stewardship rather than treating access alone as the thing to monetize. Public copy now reflects this directly: "more like a utility than a slot machine."
- **Multi-community expansion vision:** The platform is designed for multi-tenancy from the start. Each community gets its own locally-rooted space, but communities connect to one another so local never means isolated. The current obligation is to get it right in Cambria Heights first; expansion follows demonstrated local value. The mission page now includes a "For everywhere else" section that plants this vision publicly.
- **Near-term feature direction:** The initial static `About` section is complete. Remaining About follow-up is richer publishing/history structure for the Journal and future institutional pages such as `Transparency` or `Support`.
...
- **Navigation / design:** Public design system is applied, nav dropdowns work, Arcade is visible only to Super Admin, and the Experiences dropdown now links directly to `/events` for the Events entry. The `/events` page also exposes a visible `Submit Event` CTA for signed-in users. In the admin sidebar, the `/admin/stores` entry is now labeled `Store Moderation` to match the dedicated store-review workflow and the redirect notice shown in the shared content queue. The initial `About` dropdown is now live in the main nav with `Mission`, `Roadmap`, and `Journal` entries.
- **Layout consistency pass:** The root public layout now provides a small global gap between the red navbar and page content via `src/app/layout.tsx`, and the custom `.container` CSS rule in `src/app/globals.css` was corrected to center horizontally without zeroing vertical margins. Public pages that had added their own outer `max-w-* mx-auto` wrappers on top of the shared container were widened so they now use the full shared content width more consistently.
- **Admin layout pass:** `src/app/admin/layout.tsx` now intentionally breaks out of the shared public container and uses full viewport width for the admin surface. The left admin sidebar was also softened from the previous near-black treatment to a lighter slate background with gentler text/hover states so dense admin screens have more usable space and a less heavy visual feel.
- **About section baseline:** The public `About` surface is live at `/about` with `Mission`, `Roadmap`, and `Journal` pages. The mission page copy has been rewritten for warmth and specificity: first-person-plural voice, grounded in Cambria Heights / the highlands of Cambria County, with the infrastructure-as-utility philosophy and multi-community expansion vision stated in plain, human language. Pillar cards, nav descriptions, and journal entry summaries were updated to match the new tone. No CMS or data model yet — content lives in `src/lib/about.ts` and page-level JSX.
...
- About section: the static public slice is live (`/about`, `/about/mission`, `/about/roadmap`, `/about/journal`) with rewritten mission copy in a warm, community-facing voice. Remaining follow-up is richer publishing/history structure, any eventual CMS/data model for journal entries, future institutional pages such as `Transparency` or `Support`, and applying the same voice refresh to the Roadmap and Journal pages.
...
28. **Container override gotcha** — The repo defines a custom `.container` in `src/app/globals.css`. Do not set it with `margin: 0 auto;` unless you intentionally want to wipe vertical margins on every Tailwind `container` usage. The current correct behavior is horizontal centering only (`margin-left/right: auto`) so page-level top spacing such as the navbar/content gap can still work.
...
38. **Current execution priority** — The deployment baseline is now complete for the current product surface. The immediate engineering priority should shift from deployment work to post-launch validation of the live loops plus the next agreed product track (`About`) rather than starting delivery/jobs or doing speculative extra Help Wanted polish without user feedback.
...
- **Deployment verification follow-up:** The previously unverified bootstrap/deployment cleanup has now been checked with a full local verification pass. `npm run lint`, `npm run test:unit`, `npm run typecheck`, and `npm run build` all pass sequentially, and CI now mirrors that same gate on GitHub. Production deployment is live on Vercel with Neon bootstrapped, Cloudflare DNS routing the final domain, Cloudflare R2 serving uploads on `cdn.highlander.today`, MaxMind geolocation verified, `NEXTAUTH_URL` aligned with `https://highlander.today`, and the active launch auth surface verified as credentials plus Google OAuth on the final production domain. Deployment should now be treated as complete unless new production issues surface.
...
- **Deployment env checker follow-up:** `scripts/check-deployment-env.ts` now provides a repo-side preflight for hosted auth/deployment config and is exposed as `npm run deploy:check-env`. It validates the presence/format of `NEXTAUTH_*` and Google OAuth vars, warns on missing Facebook vars when Facebook login is intentionally deferred, and warns on missing MaxMind/R2 config without echoing secret values. The earlier local failure on `NEXTAUTH_URL=http://localhost:3000` was resolved before deployment was marked complete.
...
- **Facebook OAuth deferral follow-up:** Facebook login has been removed from the active auth route and the public login/register UI because Meta business verification is still pending. The deployment env checker and repo docs were updated to treat missing Facebook vars as non-blocking while this provider is disabled, and a repo-wide `npm run typecheck` pass succeeded after the change. Future sessions should treat Facebook OAuth as explicitly deferred post-launch or until verification is complete, not as a current production blocker.
...
- Completion status: complete for the initial production launch baseline.
- Verified launch checks:
  1. Production `NEXTAUTH_URL` and hosted auth env vars are aligned with the final production domain (`https://highlander.today`)
  2. Credentials login plus Google OAuth are the verified active launch auth methods on `https://highlander.today`
