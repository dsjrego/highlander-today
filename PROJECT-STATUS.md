...
> **Last updated:** 2026-03-27 (session 79 — reshaped the homepage into a two-column layout with a full-width left-column hero plus `Other News`, moved featured article category/date pills into the media panel for both image and no-image states, added author avatar support to homepage featured stories, and replaced the right column with square `Coming Soon` placeholder cards for Events and Marketplace using the shared admin-card structure on the homepage surface)
