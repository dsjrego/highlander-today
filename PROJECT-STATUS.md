...
> **Last updated:** 2026-03-26 (session 72 — added optional page-header icons to `InternalPageHeader`, wired profile/edit and profile detail headers to use the user photo/avatar at the left of the header content, tightened shared page-header spacing, removed the redundant inner edit-card subheader, and finalized the shared button-border fix by deleting the stale lower-file `.btn` reset/legacy variants that had been overriding the newer shared button system)
...
- **Profile:** `/api/profile` and `/api/users/[id]` are live. Edit supports first/last name, DOB, bio, and profile photo. Identity-locked users cannot change name or DOB. The live `/profile/edit` page now uses the lighter auth-card treatment instead of the older dark card shell: `InternalPageHeader` now supports optional description copy and optional left-side icon/avatar, the edit card itself mirrors the login email-card styling, the form is a two-column desktop layout with the profile-photo editor on the left and fields/actions on the right, locked fields use inline forbidden icons plus hover tooltips instead of repeated helper text, and account-level buttons (`Change Password`, `Email Preferences`, `Deactivate Account`) were removed from edit and moved onto the owner’s `/profile/[id]` page. The public/owner profile page now also starts with the same shared page-header component and shows the profile photo/initials avatar in that header.
...
- **Shared internal page heroes:** `src/components/shared/InternalPageHeader.tsx` is now the standard compact red-gradient page hero/section-header component for the slimmer internal/public top-of-page treatment. It now aligns with the design-system vocabulary in `DESIGN-SYSTEM-ARCHITECTURE.md`, using shared `page-header`, `page-header-inner`, `page-header-main`, `page-header-icon`, `page-header-content`, `page-label`, `page-title`, and `page-actions` classes from `src/app/globals.css`. The white uppercase page-title treatment from `Local Life` is now the default shared look, so the Support pages and other routes using `InternalPageHeader` inherit the same top-bar pattern more consistently. The component now also supports optional description copy below the title and an optional left-side icon/avatar ahead of the header copy, in addition to the optional action area, optional label, and per-page title color override. `Home`, `Local Life`, `Messages`, `My Articles`, `Roadmap` manage/submit, `Help Wanted` manage/submit, `My Stores`, `Create a Store`, `Events`, `Marketplace`, `Profile` edit, `Experiences`, the owner/public profile page, and the Support routes now use this pattern or are aligned to it.
