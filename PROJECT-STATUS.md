...
> **Last updated:** 2026-03-29 (session 88 — shipped the first in-app admin content-architecture/reference area, added explicit `Category.contentModel` support plus archived/delete cleanup in admin categories, and converted the public navbar to read top-level categories and child dropdowns dynamically from the DB while keeping `Home` fixed)
