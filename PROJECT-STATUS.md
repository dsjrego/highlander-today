...
> **Last updated:** 2026-03-26 (session 73 — turned the live profile page into a tabbed `About` / `Marketplace` / `Experiences` / `Local Life` / `Help Wanted` / owner-only `Account Settings` / owner-only `Recent Activity` surface, moved owner/report/vouch controls into the shared page-header action area, added a reusable trusted-avatar checkmark + click-to-enlarge profile-photo dialog, standardized warning/error/message/success surfaces with shared status-message styling, added DOB-blocked vouch dialogs plus system-generated internal messages when a user must enter DOB before someone can vouch for them, tightened the profile-header spacing/layout substantially beyond the earlier shared header baseline, and then replaced the old `Media` aggregate tab with dedicated content tabs that now use a shared two-card pattern where visitors see a single full-width public content card and owners see an additional management-oriented card that currently mirrors the same items while the future management flows are still directional)
...
- **Profile:** `/api/profile` and `/api/users/[id]` are live. Edit supports first/last name, DOB, bio, and profile photo. Identity-locked users cannot change name or DOB. The live `/profile/edit` page now uses the lighter auth-card treatment instead of the older dark card shell: `InternalPageHeader` supports optional description copy, optional left-side icon/avatar, and reusable header action buttons; the edit card mirrors the login email-card styling; the form is a two-column desktop layout with the profile-photo editor on the left and fields/actions on the right; locked fields use inline forbidden icons plus hover tooltips instead of repeated helper text. The public/owner `/profile/[id]` page is now a tabbed surface with `About`, `Marketplace`, `Experiences`, `Local Life`, `Help Wanted`, owner-only `Account Settings`, and owner-only `Recent Activity`. `Edit Profile` is now a header action visible only to the owner, `Report User` is a header action for non-owners, and the About tab currently focuses on DOB/trust guidance plus the bio rather than repeating identity metadata already visible in the header. The four content tabs use the same layout pattern: a public-facing content card that shows the profile owner’s items to all viewers, and an additional owner-only management card that currently mirrors those same items for future marketplace, experiences, Local Life, and Help Wanted management flows. The earlier large count/counter blocks in those tabs were removed; the current live direction favors content cards over oversized stat tiles. Help Wanted entries on the profile now use real `helpWantedPosts` data, and those posts also feed into the owner-only recent-activity tab.
