...
- **About section baseline:** The first public institutional `About` surface is now implemented at `/about` with initial static `Mission`, `Roadmap`, and `Journal` pages. This establishes the agreed top-level nav area and shared institutional copy baseline without introducing a new CMS or data model yet.
...
- About section: the first static public slice is now live (`/about`, `/about/mission`, `/about/roadmap`, `/about/journal`). Remaining follow-up is richer publishing/history structure, any eventual CMS/data model for journal entries, and future institutional pages such as `Transparency` or `Support`.
