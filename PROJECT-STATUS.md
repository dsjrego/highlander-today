...
> **Last updated:** 2026-04-22 (session 167)
...
>
> **Open page-header note:** the shared `InternalPageHeader` has now received another normalization pass: action-space is reserved by default, major route-level title-color overrides were removed, desktop headers get a small top margin, and `Directory` / `Help Us Grow` now use the same surrounding spacing pattern as the other public section pages. Treat the shared page-header system as materially improved, but still worth a quick browser/device QA pass before considering it complete.
>
...
- Do a short real-world QA pass on the updated reporter analysis flow: confirm fresh `SOURCE_PACKET_SUMMARY` runs now produce honest `brief-ready` / `draft-ready` / `needs more sourcing` guidance, and verify weak-packet event announcements no longer degrade into generic pseudo-analysis or raw-URL dumps.
...
- Keep using the generated theme-manifest registry path: add manifests under `src/lib/theme/manifests/`, let `scripts/generate-theme-manifests.js` rebuild `src/lib/theme/generated-manifests.ts`, and avoid hand-maintaining `src/lib/theme/registry.ts`.
