...
> **Last updated:** 2026-04-04 (session 111 — documented organization inbox / CRM planning plus the latest directory behavior, trust gating, and deferred self-serve organization listing guidance)
...
- **Events:** `/events`, `/events/[id]`, `/events/submit`, `/api/events`, and `/api/events/[id]` are live on the current schema. Submissions go to `PENDING_REVIEW`; staff can approve/reject them. Public events use real data and the submit page matches the current public shell. `/admin/events` now also includes a compact `+ Event` create tab for direct staff/admin creation, supports initial status selection, and can optionally link an event to an `Organization` through an inline filtered selector. Event locations are now modeled as first-class shared `Location` records: `Event` carries required `locationId` plus optional `venueLabel`, both admin/public event forms create or select reusable structured locations inline, and event read surfaces now render canonical location data rather than a raw `locationText` string.
...
10. **Event location/organization schema note:** the current event forms depend on `locations`, `events.locationId`, and `events.organizationId`. If admin/public event creation or event reads fail on any of those fields in any environment, the database is behind the schema; run `npm run db:push` / `npx prisma db push --schema prisma/schema.prisma` before debugging the UI.
...
- **Events:** traced end-to-end through creation, moderation, public listing, detail view, and navigation surfacing. The old scaffold-era event API route was replaced first with the current schema fields (`submittedByUserId`, `startDatetime`, `endDatetime`, `locationText`, `photoUrl`, `costText`, `contactInfo`), and now the active event location layer has moved again to shared `Location` records plus `Event.locationId` and optional `Event.venueLabel`.
