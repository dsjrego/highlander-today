...
- **Verification baseline:** ESLint is configured; `npm run lint` now passes cleanly without warnings; `npm run test:unit` is green on the current suite (`permissions`, `trust-cascade`, `marketplace-status`); `npm run typecheck` passes repo-wide again; `npm run build` succeeds; CI mirrors `lint`, `test:unit`, `typecheck`, and `build` on PRs and pushes to `main`.
...
- **Directory messaging:** people rows on `/directory` now expose a client-side `Message` action in the contact column for authenticated viewers. It reuses the compact inline dialog pattern from `/admin/users`, submits through `POST /api/messages`, and redirects into the resulting conversation thread. Signed-out viewers see a sign-in prompt instead, and the current user sees `Your listing` on their own row.
