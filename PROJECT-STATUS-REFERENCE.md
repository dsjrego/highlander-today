...
> **Last updated:** 2026-04-17 (session 164 — documented reporter Phase 1 workflow polish, live Anthropic drafting, and the first Reporter Agent analysis tab)
...
>
> **Open page-header note:** the shared `InternalPageHeader` has now received another normalization pass: action-space is reserved by default, major route-level title-color overrides were removed, desktop headers get a small top margin, and `Directory` / `Help Us Grow` now use the same spacing/action-slot pattern as the other public section pages. Treat the shared page-header system as materially improved rather than still blocked, but keep a short browser/device QA pass on the list before declaring it complete.
...
>
> **Session 153 reference note:** article management and homepage image handling received a focused operational pass. On article detail pages, the hero image now stays proportional but is visually reduced/capped and centered, and the article body column is wider. In preview/editing flows, the non-editorial `ArticlePreview` header now consumes tenant theme tokens like the recipe preview rather than fixed Highlander colors. The shared article editor at `/local-life/submit` now supports edit mode through `?edit={articleId}` by loading existing article data and saving through `PATCH /api/articles/[id]`; owner profile `Articles` now includes `DRAFT`, `PENDING_REVIEW`, and `UNPUBLISHED` entries only for the profile owner and links non-approved articles directly into that editor; and the editor header shortcut now routes to `/profile/[id]?tab=articles` rather than the stale `tab=local-life` path. Admin article cleanup is now possible directly from `/admin/articles` through row-level delete, and the confirmation uses the platform’s native popover dialog pattern rather than `window.confirm`. Production homepage hero/recipe thumbnails were also debugged: the cards use `next/image` while article/recipe detail pages use plain `img`, so uploads on the live CDN host (`cdn.highlander.today`) were silently blocked on homepage cards until `next.config.mjs` was updated to use `images.remotePatterns` for the real production/custom CDN hosts. Existing content did not need to be recreated; the fix is config-only and requires a deploy/restart.
>
> **Session 154 reference note:** a follow-up visual polish pass tightened the standard article-detail composition and the homepage featured-image overlay. The standard `/local-life/[id]` article path now uses a single shared main-column width for header, hero, body, and footer rather than letting title/image/body edges drift independently. The standard hero overlay now holds the optional image caption above the category and author/date metadata, the separate caption block below the image was removed, and the duplicate date/author block above the article body was removed so the body starts directly under the image. The article body left edge now overrides the older centered `.article-card-content` rule that came from the editorial stylesheet, preventing the lingering offset seen after the first width pass. On the homepage featured card, the image darkening overlay was strengthened, the pills now sit inside a blurred dark container instead of floating directly on the photo, and the subcategory pill contrast was increased after bright portraits still made the category label difficult to read even after the first overlay adjustment.
>
> **Session 155 reference note:** the homepage and directory/organization flows both moved from provisional patterns toward real reusable product structure. Prisma now includes `HomepageBox` and `HomepageBoxItem`, while the old homepage-section tables remain only as temporary legacy import compatibility. `src/lib/homepage.ts`, `/api/homepage/sections`, `/admin/homepage`, and the public `/` homepage now all operate on ordered homepage boxes with one hero item plus up to five optional supporting links under one shared visual template. The public homepage renders those boxes in sort order so any domain can become the lead hero just by moving its box to slot one; supporting links now render as compact list rows with thumbnails and publish dates, and the old explanatory placeholder sentences were removed. The directory/organization slice also now includes a real public `/organizations/submit` flow: signed-in users can submit a new pending organization or request ownership of an approved one through a pending owner membership, and `/directory` exposes a `Your Organization` page-header action that routes trusted-capable users into that flow while showing anonymous/non-trusted users a native no-JavaScript trusted-user requirement popover instead of a broken CTA. The article detail sidebar also gained a share button above the `Author` heading that copies the current URL and shows a transient confirmation, and the shared `InternalPageHeader` defaults were tightened again so public section pages behave more uniformly.
>
> **Session 164 reference note:** reporter Phase 1 was materially polished and partially extended into the next agent layer. The admin reporter detail surface at `/admin/reporter/[id]` now behaves more like a real newsroom operations tool: `Sources`, `Blockers`, and `Drafts` use the same compact row/list pattern with visible trailing controls; source rows now support add/edit/delete in the UI; blocker codes are normalized via a dropdown rather than free text; draft rows now show sequence labels, timestamps, provider/model metadata, and real `View draft` behavior instead of routing every row back into the linked article. Converted runs are no longer treated as dead ends — a run can keep its linked `Article` draft and still generate additional reporter drafts for comparison. The Anthropic provider moved from fallback stub to live integration through `ANTHROPIC_API_KEY` and `REPORTER_MODEL_PROVIDER=anthropic`; silent fallback-on-error was removed so missing keys or API failures now surface as real errors, while the draft table’s `Model` column makes it obvious which rows are genuine Anthropic generations. The first Reporter Agent slice now exists using the existing `ReporterDraft` model with `draftType = SOURCE_PACKET_SUMMARY`, surfaced in a dedicated `Analysis` tab that can generate and inspect internal analysis artifacts separately from public-facing article drafts. Current open issue: on thin packets (for example, a short event notice plus one source URL), Anthropic-backed analysis still tends to collapse into weak/generic pseudo-analysis, so the next recommended reporter work is a stronger sufficiency/recommendation layer that explicitly classifies runs as brief-ready, draft-ready, needs-sourcing, or blocked rather than trying to force expansive analysis out of weak material.
...
`prisma/seed.ts` creates community `Highlander Today` (slug `highlander-today`), default homepage boxes, and default site settings. Create the initial Super Admin explicitly with:
...
- **Shared internal page heroes:** `src/components/shared/InternalPageHeader.tsx` is now the standard compact red-gradient page hero/section-header component for the slimmer internal/public top-of-page treatment. It aligns with the design-system vocabulary in `DESIGN-SYSTEM-ARCHITECTURE.md`, using shared `page-header`, `page-header-inner`, `page-header-main`, `page-header-icon`, `page-header-content`, `page-label`, `page-title`, and `page-actions` classes from `src/app/globals.css`. The white uppercase page-title treatment from `Local Life` is now the default shared look, so the Support pages and other routes using `InternalPageHeader` inherit the same top-bar pattern more consistently. The component now also supports optional description copy below the title, an optional left-side icon/avatar ahead of the header copy, optional action area, optional label, and per-surface class overrides so special headers like the live profile page can be tightened beyond the shared default without forking the component. More recently, the defaults were normalized again so action-space is reserved by default, major route-level title-color overrides were removed, and desktop headers get a small shared top margin rather than route-by-route spacing hacks.
...
- **Homepage:** `src/app/page.tsx` + `src/lib/homepage.ts` SSR-render the homepage using the ordered `HomepageBox` / `HomepageBoxItem` model. Homepage/community resolution now routes through the shared tenant-domain helper in `src/lib/tenant.ts`, still preserving localhost fallback plus a legacy `Community.domain` fallback for existing records. The live homepage uses the `Youth Local` visual direction and starts with the same compact `InternalPageHeader` hero style used across the rest of the public site. The main content is now a sorted stream of reusable homepage boxes rather than rigid hardcoded lanes: each box has one hero item, up to five optional supporting links, one shared visual template, visibility/order controls, and a content type such as article, recipe, event, marketplace, or future domains. The first box is emphasized visually, and the rest follow in grid order while preserving mobile stacking order. Supporting links now render as compact list rows with thumbnails and dates, and the extra explanatory placeholder copy was removed. Legacy homepage sections remain in the schema only as a temporary compatibility/import bridge so old curated pins can populate the new box model on first load.
...
- **Public organization submit/claim:** `/organizations/submit` is now live as the first ordinary-user-facing organization self-serve surface. Signed-in users can create a new `PENDING_APPROVAL` organization through the existing public organizations API or search approved organizations and request ownership through `POST /api/organizations/[id]/claim`, which creates a pending `OWNER` membership request for review. The first pass is intentionally simple: it handles submission and claim request creation, not the richer owner-management or claim-review tooling that still needs to be built around it.
...
- **Verification baseline:** ESLint is configured; `npm run lint` now passes cleanly without warnings; `npm run test:unit` is green on the current suite (`permissions`, `trust-cascade`, `marketplace-status`, `event-series`, `event-route-recurring`, `event-series-mutations`, `event-datetime`); `npm run typecheck` passes repo-wide again; `npm run build` succeeds; CI mirrors `lint`, `test:unit`, `typecheck`, and `build` on PRs and pushes to `main`.
...
- **Homepage curation:** `/admin/homepage` and `/api/homepage/sections` are still the active curation surface, but they now operate on the real homepage box model rather than the old section-bucket split. Editors manage an ordered list of boxes, each box has one hero item plus up to five optional supporting links, and the same visual/content contract applies across articles, recipes, events, marketplace, and future domains. The homepage can therefore promote any content type into the top hero position simply by moving that box to sort position one rather than by treating articles as the permanently special case. Legacy homepage sections remain in the schema only as a temporary compatibility/import bridge, and `src/lib/homepage.ts` will import old curated section data into homepage boxes on first read in environments that still have the older records. The earlier production issue where `/admin/homepage` rendered only the static title/description/save button was traced to a blank production `communities.domain`; this was fixed by setting the production domain and is one of the reasons the newer tenant-domain foundation now exists.
...
- **Admin dashboard/settings cleanup:** `/admin` no longer shows fake summary numbers for events, marketplace listings, pending review, or recent activity. Those dashboard cards now read real counts from the current community-scoped content/store models plus recent `ActivityLog` rows. `/admin/settings` also no longer simulates success with a placeholder timeout; it now loads and saves against the existing `/api/settings` route.
...
- **Organization self-serve rule:** the old deferral no longer applies because `/organizations/submit` now exists. Keep the current product rule instead: the `Your Organization` CTA may be visible in the directory header, but only trusted-capable users should be able to enter the submit/claim flow directly; anonymous or merely registered users should continue to see the native trusted-user requirement popover rather than a dead-end route.
...
6. **Milestone 6 — Organization foundation + directory/org-site MVP:** partially complete. `Organization`, `OrganizationMembership`, structured organization child models, profile opt-in, `/admin/organizations`, the live `/directory` browse surface, public organization detail pages, and a first public `/organizations/submit` submit/claim flow now exist. Remaining work is richer organization editing/management, claim-review tooling, owner self-management after claim approval, and the later org-site/subdomain layer such as `flavortown.highlander.today` without treating marketplace stores as the general organization abstraction.
...
11. Homepage real data by box model: done for the current per-community model
12. Sister-site content pull-through: not implemented
13. First-class organizations / directory foundation: partially complete. Use the dedicated `Organization` + `OrganizationMembership` model rather than widening `Store`; the current live layer already supports directory listings, membership/accountability, admin creation/moderation, public organization detail pages, and a first public submit/claim request path, while richer owner self-management, claim-review hardening, and future paid org subdomain routing are still pending.
...
- **Homepage:** verified domain-aware community resolution, ordered homepage-box rendering, legacy homepage-section import compatibility, supporting-link thumbnail/date rows, `next/image`, and removal of old placeholder helper copy.
...
`prisma/seed.ts` creates only structural data: community, categories, homepage boxes, and site settings. It is safe to re-run idempotently.
