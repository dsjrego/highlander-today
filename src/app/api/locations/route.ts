...
function canManageLocations(userRole: string) {
  return checkPermission(userRole, 'events:create');
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!canManageLocations(userRole)) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (duplicates.length > 0 && !validated.forceCreate) {
      return NextResponse.json({ error: 'Possible duplicate location found', duplicates }, { status: 409 });
    }
